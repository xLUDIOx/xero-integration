...
export class PayhawkBankExpenseModelBuilder implements IPayhawkExpenseModelBuilder {
    constructor(
    ...
    async build(payhawkExpense: IValidatedExpense): Promise<IPayhawkExpenseModel> {
        ...
        let taxAmount: Optional<number>;
        if (useExpenseCurrency) {
            fxRate = 1.0;

            currency = expenseCurrency;
            totalAmount = expenseAmount;
            taxAmount = payhawkExpense.reconciliation.expenseTaxAmount;
        } else {
            fxRate = calculateConversionRate({ from: expenseAmount, to: paymentAmount });
            currency = paymentCurrency;
            totalAmount = paymentAmount;
            taxAmount = payhawkExpense.reconciliation.expenseTaxAmount !== undefined ?
                multiplyAmounts(payhawkExpense.reconciliation.expenseTaxAmount, fxRate) :
                undefined;
        }
        ...
        return {
            ...
            dueDate: payhawkExpense.paymentData?.dueDate,
            payments: payhawkExpense.payments.map(p => {
                const paymentFxRate = calculateConversionRate({ from: expenseAmount, to: p.amount });
                return {
                    id: p.id,
